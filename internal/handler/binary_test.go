...
func TestBinaryHandler_AddBinary_Success(t *testing.T) {
	...
	testUploadInfo := minio.UploadInfo{Bucket: testvars.TestUserID}
	...
	resp := uploadBinaryFile(t, ts, http.MethodPost, "/api/binary/add", testvars.TestFileName, testvars.TestBinaryData, true)

	body, _ := io.ReadAll(resp.Body)
	defer resp.Body.Close()

	assert.Equal(t, 201, resp.StatusCode, "Response status code didn't match expected")
	assert.Equal(t, "File "+testvars.TestFileName+" uploaded successfully", string(body), "Response text didn't match expected")
}
...
func TestBinaryHandler_AddBinary_Unauthorized(t *testing.T) {
	...
	resp := uploadBinaryFile(t, ts, http.MethodPost, "/api/binary/add", testvars.TestFileName, testvars.TestBinaryData, false)
...
func TestBinaryHandler_GetBinary_Unauthorized(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodPost, ts.URL+"/api/binary/get", strings.NewReader(testvars.TestDataID))
...
func TestBinaryHandler_DeleteBinary_Success(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodPost, ts.URL+"/api/binary/delete", strings.NewReader(testvars.TestDataID))
	assert.NoError(t, err, "failed to create request")
	...
	assert.Equal(t, "Binary data "+testvars.TestDataID+" deleted successfully", string(body), "Response text didn't match expected")
}
